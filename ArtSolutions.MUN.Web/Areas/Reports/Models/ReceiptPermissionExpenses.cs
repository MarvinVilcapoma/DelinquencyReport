...
{
    public class ReceiptPermissionExpenses
    {
        #region public methods  
        public ReceiptPermissionExpensesModel Get()
        {
        ...
        public ReceiptPermissionExpensesModel Get(DateTime? startDate, DateTime? endDate, decimal? balanceFrom, decimal? balanceTo, string commaSeperatedAccountIds, string commaSeperatedCashierIds, JQDTParams jqdtParams)
        {
            ReceiptPermissionExpensesModel model = new ReceiptPermissionExpensesModel();
            if (jqdtParams != null)
                model = GetByPaging(startDate, endDate, balanceFrom, balanceTo, commaSeperatedAccountIds, commaSeperatedCashierIds, jqdtParams);
            model.ReceiptPermissionExpensesList = model.ReceiptPermissionExpensesList ?? new List<ReceiptPermissionExpensesModelList>();
            model.ReportCompanyModel = GetReportCompany(startDate, endDate);
            return model;
        }

        public ReceiptPermissionExpensesModel GetExportLayout(DateTime? startDate, DateTime? endDate, decimal? balanceFrom, decimal? balanceTo, string commaSeperatedAccountIds, string commaSeperatedCashierIds)
        {
        ...
        {
            return new ReportCompanyModel().Get(ArtSolutions.MUN.Web.Resources.Report.ReceiptPermissionExpensesTitle, ArtSolutions.MUN.Web.Resources.Report.DepartmentOfFinance, 7, FromDate, ToDate);
        }

        private ReceiptPermissionExpensesModel GetByPaging(DateTime? startDate, DateTime? endDate, decimal? balanceFrom, decimal? balanceTo, string commaSeperatedAccountIds, string commaSeperatedCashierIds, JQDTParams jqdtParams)
        {
            ...
            return new RestSharpHandler().RestRequest<ReceiptPermissionExpensesModel>(APISelector.Municipality, true, "api/Report/ReceiptPermissionExpenses", "GET", lstParameter);
        }
        #endregion
    }
}