...
{
    public class ReceiptPermissionStreetVendors
    {
        #region public methods  
        public ReceiptPermissionStreetVendorsModel Get()
        {
        ...
        public ReceiptPermissionStreetVendorsModel Get(DateTime? startDate, DateTime? endDate, decimal? balanceFrom, decimal? balanceTo, string commaSeperatedAccountIds, string commaSeperatedCashierIds, JQDTParams jqdtParams)
        {
            ReceiptPermissionStreetVendorsModel model = new ReceiptPermissionStreetVendorsModel();
            if (jqdtParams != null)
                model = GetByPaging(startDate, endDate, balanceFrom, balanceTo, commaSeperatedAccountIds, commaSeperatedCashierIds, jqdtParams);
            model.ReceiptPermissionStreetVendorsList = model.ReceiptPermissionStreetVendorsList ?? new List<ReceiptPermissionStreetVendorsModelList>();
            model.ReportCompanyModel = GetReportCompany(startDate, endDate);
            return model;
        }

        public ReceiptPermissionStreetVendorsModel GetExportLayout(DateTime? startDate, DateTime? endDate, decimal? balanceFrom, decimal? balanceTo, string commaSeperatedAccountIds, string commaSeperatedCashierIds)
        {
        ...
        {
            return new ReportCompanyModel().Get(ArtSolutions.MUN.Web.Resources.Report.ReceiptStreetVendorTitle, ArtSolutions.MUN.Web.Resources.Report.DepartmentOfFinance, 7, FromDate, ToDate);
        }

        private ReceiptPermissionStreetVendorsModel GetByPaging(DateTime? startDate, DateTime? endDate, decimal? balanceFrom, decimal? balanceTo, string commaSeperatedAccountIds, string commaSeperatedCashierIds, JQDTParams jqdtParams)
        {
            ...
            return new RestSharpHandler().RestRequest<ReceiptPermissionStreetVendorsModel>(APISelector.Municipality, true, "api/Report/ReceiptStreetVendors", "GET", lstParameter);
        }
        #endregion
    }
}