...
$(document).on('click', '#btnGo', function () {
    if (validateForm()) {
        InitializeDataTable("tblReceiptCommercialFacilitiesRental");
        return true;
    }
    return false;
});
...
function InitializeDataTable(tableName) {
    ...
    $('#' + tableName).dataTable({
        ...
        "scrollCollapse": true,
        "ajax": {
            "url": ROOTPath + "/Reports/Report/ReceiptCommercialFacilitiesRental",
            "type": "POST",
            "data": function (data) {
        ...
        "columns": [
            ...
            {
                name: "RegisterNumber", title: identitymunicipal, "data": "RegisterNumber", className: "col-lg-2"
            },
            {
                name: "ServiceName", title: commercialFacility, "data": "ServiceName", className: "col-lg-2 text-description-field"
            },
            {
                name: "Description", title: description, "data": "Description", className: "col-lg-2 text-description-field"
            },
            {
                name: "total", title: total, "data": "FormattedTotalPayment", className: "col-lg-1 text-right"
            }
        ],
        ...
        rowGroup: {
            ...
            endRender: function (rows, group) {
                ...
                return $('<tr class="sub-header font-bold">')
                    .append('<td colspan="6" class="p-l-sm-td text-right">' + total + " - " + rows.data().pluck("FormattedDate")[0] + '</td>')
...
function Print() {
    if (validateForm()) {
        $.ajax({
            type: "GET",
            url: ROOTPath + "/Reports/Report/PrintReceiptCommercialFacilitiesRental",
            data: {
