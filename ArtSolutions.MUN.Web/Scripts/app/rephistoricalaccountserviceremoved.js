...
function initDataTable() {
    $('#historicalAccountServiceRemovedTable').DataTable({
        "paging": false,
        "ordering": false,
        "info": false,
        "searching": false,
...
function InitializeData() {
    $.ajax({
        type: "POST",
        url: ROOTPath + "/Reports/Report/HistoricalAccountServiceRemoved",
        data: {           
        ...
        success: function (response) {
            if (response.status) {
                hideLoading();
                $("#divHistoricalAccountServiceRemoved").html("").html(response.data);
                initDataTable();
                $('#historicalAccountServiceRemovedTable .table-description-field').tooltip({
                    container: "body",
                    html: true
                });
            }
...
function Print() {
    if (validateForm()) {
        ...
        $.ajax({
            type: "GET",
            url: ROOTPath + "/Reports/Report/PrintHistoricalAccountServiceRemoved",
            data: {               
                'startDate': new Date($("#StartDate").datepicker('getDate')).toDateString()
                , 'endDate': new Date($("#EndDate").datepicker('getDate')).toDateString()
            },
