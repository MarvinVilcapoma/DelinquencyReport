...
function pushSelectedItems() {
    selectedItems = [];
    if ($(".debitnoteitem").length > 0) {
        $(".debitnoteitem").each(function (index) {
            if ($(this).is(':checked')) {
...
function pullSelectedItems() {
    $.each(selectedItems, function (index, el) {
        $("tr[id='" + selectedItems[index] + "']").find(".debitnoteitem").prop("checked", true);
    });
}
...
function getPaymentOptionTotal() {
    ...
    total = 0;
    $(".debitnoteitem:checked").each(function (index) {
        if ($(this).closest('tr').find('.hdnAmount').val() > 0)
            total = total + parseFloat($(this).closest('tr').find('.hdnAmount').val());
    });
    if ($("#ApplyCreditAmount").val() > 0 && $("#tblPaymentOptions .amountinput").length == 1) {
        total = total - GlobalConvertToDecimal($("#ApplyCreditAmount").val())
    }
...
function loadDebitNotes() {
    ...
    pushSelectedItems();
    $.ajax({
        url: ROOTPath + "/Payment/GetDebitNotes",
        type: "post",
        data:
        {
            "accountId": accountId,
        ...
        success: function (response) {
            if (response.status == undefined) {
                $("#DebitNoteRows").html('').html(response);
...
function loadDebitNotesOnly() {
    ...
    pushSelectedItems();
    $.ajax({
        url: ROOTPath + "/Payment/GetDebitNotesOnly",
        data:
        {
            "accountId": accountId
        },
...
function confirmPayment(actionType) {
    ...
    if ($("#frmPayment").valid()) {
        swal({
            ...
            if (confirmed) {
                if (actionType == "debitnotesave") {
                    $("#btnSave").prop("type", "submit").click().prop("type", "button");
                }
                if (actionType == "debitnotesaveandcontinue") {
...
function paymentSuccessCallback(response) {
    if (!response.status) {
        ...
        if (response.actionType == "debitnotesaveandcontinue") {
            url = (ROOTPath + "/Collections/Payment/Add?ActionType=debitnote");
        }
        else {
        ...
        if (Isprint == true) {
            if (response.paymentID != undefined && response.paymentID != null && response.paymentID > 0) {
                Printurl = (ROOTPath + "/Collections/Payment/RollPrint?ID=" + response.paymentID + "&Type=DebitNote&TypeID=" + response.serviceTypeId);
