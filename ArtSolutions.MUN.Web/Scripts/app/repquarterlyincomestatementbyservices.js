...
function InitializeDataTable() {
    $('#tblQuarterlyIncomeStatementByServices').DataTable({
        "scrollY": "100vh",
        "scrollX": true,
        scrollCollapse: true,
        "paging": false,
...
function InitializeData() {
    $.ajax({
        type: "POST",
        url: ROOTPath + "/Reports/Report/QuarterlyIncomeStatementByServices",
        data: {
            'Quarter': $("#Quarter").val(),
            'Year': $("#Year").val(),
        ...
        success: function (response) {
            if (response.status) {
                hideLoading();
                $("#divQuarterlyIncomeStatementByServices").html("").html(response.data);
                InitializeDataTable();
                rowCount = pageSize;
            }
            else {
...
function loadMoreQuarterlyIncomeStatementByServices(element) {
    ...
    $.ajax({
        ...
        success: function (response) {
            $(response.data).insertBefore("#tbodyQuarterlyIncomeStatementByServices");
            ...
            if (rowCount < $("#TotalRecord").val()) {
                $("#tbodyQuarterlyIncomeStatementByServices").show();
            }
            else {
                $("#tbodyQuarterlyIncomeStatementByServices").hide();
            }
        }
    });
}
...
function Print() {
    $.ajax({
        type: "GET",
        url: ROOTPath + "/Reports/Report/PrintQuarterlyIncomeStatementByServices",
        data: { 'Quarter': $("#Quarter").val(), 'Year': $("#Year").val(), 'serviceIds': getServiceCodes() },
        beforeSend: function () {
            showLoading();
        },
        success: function (response) {
