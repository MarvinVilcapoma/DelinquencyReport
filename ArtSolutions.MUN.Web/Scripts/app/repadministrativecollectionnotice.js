...
function initializeDatePicker() {
    $(".notificationExpirationDate").datepicker({
...
$('.notificationExpirationDate').keypress(function (e) {
    e.preventDefault();
});
...
function validateAdvanceSearch() {
    ...
    if ($("#txtNotificationExpirationDate").val() == "" || $("#txtRepresentativesName").val().trim() == "") {
...
function InitializePropertyTable() {
    if ($("#accountPropertyListTable tbody tr.no-data").length == 1) {
        return false;
    }
    accountPropertyListTable = $('#accountPropertyListTable').DataTable({
        "paging": false,
        "ordering": false,
        "info": false,
        "searching": false,
...
function InitializeData() {
    if ($("#AccountId").val() > 0) {
        ...
        $.ajax({
            type: "POST",
            url: ROOTPath + "/Reports/Report/AdministrativeCollectionNotice",
            data: {
...
function Print(data) {
    if (checkAccountIdInput() && $("#form").valid() && validateForm()) {
        $.ajax({
            type: "GET",
            url: ROOTPath + "/Reports/Report/PrintAdministrativeCollectionNotice",
            data:
            {
                'accountId': $("#AccountId").val(),
                'notificationExpirationDate': new Date($("#txtNotificationExpirationDate").datepicker('getDate')).toDateString(),
