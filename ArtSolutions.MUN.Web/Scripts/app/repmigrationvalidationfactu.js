...
function InitializeDataTable() {
    $('#tblMigrationValidationFactu').DataTable({
        "scrollY": "100vh",
        "scrollX": true,
        scrollCollapse: true,
        "paging": false,
...
function InitializeData() {
    $.ajax({
        type: "POST",
        url: ROOTPath + "/Reports/Report/MigrationValidationFactu",
        data: {
            'typeID': $("#ddlType").val(),
        ...
        success: function (response) {
            if (response.status) {
                hideLoading();
                $("#divMigrationValidationFactu").html("").html(response.data);
                InitializeDataTable();
                rowCount = pageSize;
            }
            else {
...
function loadMoreMigrationValidationFactu(element) {
    ...
    $.ajax({
        ...
        success: function (response) {
            $(response.data).insertBefore("#tbodyMigrationValidationFactu");
            ...
            if (rowCount < $("#TotalRecord").val()) {
                $("#tbodyMigrationValidationFactu").show();
            }
            else {
                $("#tbodyMigrationValidationFactu").hide();
            }
        }
    });
}