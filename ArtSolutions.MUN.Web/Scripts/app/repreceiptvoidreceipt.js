...
$(document).on('click', '#btnGo', function () {
    if (validateForm()) {
        InitializeDataTable("tblVoidRegister");
        return true;
    }
    return false;
});
...
function InitializeDataTable(tableName) {
    ...
    $('#' + tableName).dataTable({
        ...
        "scrollCollapse": true,
        "ajax": {
            "url": ROOTPath + "/Reports/Report/VoidReceipts",
            "type": "POST",
            "data": function (data) {
        ...
        "columns": [
            ...
            {
                name: "VoidDate", title: Voiddate, "data": "FormattedVoidDate", className: "col-lg-2"
            },
            {
            ...
            {
                name: "VoidReason", title: CancellationReason, "data": "VoidReason", className: "col-lg-3 text-description-field"
            },
            {
                name: "TotalPayment", title: CancellationAmount, "data": "FormattedTotalPayment", className: "col-lg-2 text-right"
            }
        ],
        ...
        "footerCallback": function (row, data, start, end, display) {
            ...
            for (var i = 0; i < display.length; i++) {
                totalCancelAmount = totalCancelAmount + data[i].TotalPayment;
            }
            $(row).find("th").eq("1").text(NumberToCultureFormat(totalCancelAmount));
...
function Print() {
    if (validateForm()) {
        $.ajax({
            type: "GET",
            url: ROOTPath + "/Reports/Report/PrintVoidRegister",
            data: {
