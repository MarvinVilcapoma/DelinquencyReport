...
function InitializeDataTable() {
    $('#tblMonthlyIncomeStatementByServices').DataTable({
        "scrollY": "100vh",
        "scrollX": true,
        scrollCollapse: true,
        "paging": false,
...
function InitializeData() {
    $.ajax({
        type: "POST",
        url: ROOTPath + "/Reports/Report/MonthlyIncomeStatementByServices",
        data: {
        ...
        success: function (response) {
            if (response.status) {
                hideLoading();
                $("#divMonthlyIncomeStatementByServices").html("").html(response.data);
                InitializeDataTable();
                rowCount = pageSize;
            }
            else {
...
function loadMoreMonthlyIncomeStatementByServices(element) {
    ...
    $.ajax({
        ...
        success: function (response) {
            $(response.data).insertBefore("#tbodyMonthlyIncomeStatementByServices");
            ...
            if (rowCount < $("#TotalRecord").val()) {
                $("#tbodyMonthlyIncomeStatementByServices").show();
            }
            else {
                $("#tbodyMonthlyIncomeStatementByServices").hide();
            }
        }
    });
}
...
function Print() {
    ...
    $.ajax({
        type: "GET",
        url: ROOTPath + "/Reports/Report/PrintMonthlyIncomeStatementByServices",
        data: { 'closingDate': $("#ClosingDate").val(), 'serviceIds': getServiceCodes() },
        beforeSend: function () {
            showLoading();
        },
        success: function (response) {
