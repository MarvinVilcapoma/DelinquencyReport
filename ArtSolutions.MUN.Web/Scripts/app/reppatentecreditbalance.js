...
function InitializeDataTable(tableName) {
    ...
    $('#' + tableName).dataTable({
        ...
        "scrollCollapse": true,  
        "ajax": {
            "url": ROOTPath + "/Reports/Report/CreditBalanceByBusinessLicense",
            "type": "POST",
            "data": function (data) {
        ...
        "columns": [             
            ...
            {
                name: "Credit", title: creditamount, "data": "FormattedCreditBalance", className: "col-lg-1 text-right"
            }
        ...
        "footerCallback": function (row, data, start, end, display) {
            ...
            for (var i = 0; i < display.length; i++) {
                ...
                totalCredit = totalCredit + data[i].CreditBalance;                
            }
            ...
            $(row).find("th").eq("8").text(NumberToCultureFormat(totalCredit));
...
function Print() {
    if (validateForm()) {
        $.ajax({
            type: "GET",
            async: false,
            url: ROOTPath + "/Reports/Report/PrintCreditBalanceByBusinessLicense",
            data: {
